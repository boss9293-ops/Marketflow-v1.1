...
export function buildStructuralInterpretation(
  ...
  const baseStatement =
    ...
    alignmentResult.alignment === 'Aligned'
      ? 'The structure is broadly supported, while constraints remain limited'                      :
    alignmentResult.alignment === 'Mixed'
      ? 'The structure is partially supported, while key constraints limit full participation'      :
  ...
  // Guard against empty support to avoid "This is driven by , while..."
  const cause =
    support.length > 0 && weakness.length > 0 ? `This is driven by ${join(support)}, while ${join(weakness)}.` :
    support.length > 0                         ? `This is driven by ${join(support)}.`                          :
    weakness.length > 0                        ? `Structural constraints include ${join(weakness)}.`            :
                                                 'Signal direction is consistent but lacks strong directional drivers.'
