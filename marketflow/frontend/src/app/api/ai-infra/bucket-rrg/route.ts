// AI 인프라 병목 버킷 RRG 경로 캐시를 반환하는 API 라우트 — Phase D-3 / D-8 benchmark-aware
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { ...PENDING_BOTTLENECK, generatedAt: new Date().toISOString(), selected_benchmark: benchmark, _source: 'pending' },
