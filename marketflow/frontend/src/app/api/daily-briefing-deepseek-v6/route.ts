...
export async function GET() {
  const data = await readCacheJsonOrNull<Record<string, unknown>>('daily_briefing_deepseek_v6.json')
  if (!data) {
    return NextResponse.json(
      {
        error: 'daily_briefing_deepseek_v6.json not found',
        rerun_hint: 'python backend/scripts/build_daily_briefing_deepseek_v6.py',
      },
      { status: 404 }
    )
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(backendApiUrl('/api/briefing/deepseek/v6/generate'), {
      method: 'POST',
