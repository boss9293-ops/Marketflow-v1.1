...
export async function GET() {
  const data = await readCacheJsonOrNull<Record<string, unknown>>('daily_briefing_v4.json')
  if (!data) {
    return NextResponse.json(
      {
        error: 'daily_briefing_v4.json not found',
...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(backendApiUrl('/api/briefing/v4/generate'), {
      method: 'POST',
