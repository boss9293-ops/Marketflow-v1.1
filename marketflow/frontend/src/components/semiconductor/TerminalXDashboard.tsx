...
export default function TerminalXDashboard() {
  ...
  return (
    ...
    >
      ...
      {/* ???? HEADER 40px ???? */}
      <header className="h-[40px] flex items-center justify-between bg-[#0a0a0a] border-b border-slate-800 px-4 md:px-6 xl:px-10 2xl:px-14 shrink-0 sticky top-0 z-50">
      ...
      <div className={`flex px-4 md:px-6 xl:px-10 2xl:px-14 gap-[10px] bg-black py-3 flex-1 ${mainTab === 'DATA_LAB' ? '' : 'hidden'}`}>
                        ...
                        <div className="mt-2 space-y-1.5">
                          {orderedSoxxBucketContributions.map((bucket) => {
                            ...
                            return (
                              <div
                                key={bucket.bucketId}
                                className="rounded-sm border border-slate-800 bg-[#0d0d0d] px-2 py-1.5"
