      ...
      <div className={`flex px-4 md:px-6 xl:px-10 2xl:px-14 gap-[10px] bg-[#020408] py-3 flex-1 ${mainTab === 'MASTER' ? '' : 'hidden'}`}>
        ...
        <aside className="w-1/4 shrink-0 flex flex-col gap-[10px] h-fit">
          ...
          {(() => {
            const stageCls =
              ...
              stageName.includes('RECOVERY') || stageName.includes('TROUGH')   ? 'text-blue-400'    : 'text-slate-300 tracking-[0.02em]'
            const stageBar =
            ...
            const contextLine =
              stageName.includes('MID EXPANSION') && breadthPct >= 70
                ? 'Broad participation is consistent with the current expansion structure.'
            ...
            return (
              <Panel title="Cycle Position" icon={Cpu}>
                <div className="flex flex-col gap-[10px]">
                  <div>
                    <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em] mb-0.5" style={{ fontFamily: UI_FONT }}>Current Stage</div>
                    <div className={`text-[15px] font-black leading-[1.6] truncate ${stageCls}`}>{stageName !== '?' ? stageName : 'Loading…'}</div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>Cycle Progress</div>
                      <span className="text-[11px] font-bold text-slate-300 tracking-[0.02em] font-mono" style={{ fontFamily: DATA_FONT }}>{cyclePos > 0 ? `${Math.round(cyclePos)}%` : '?'}</span>
                    </div>
                  ...
                  {live && (
                    <p className="text-[11px] text-slate-300 tracking-[0.02em] leading-[1.6] border-t border-slate-800/60 pt-1.5">{contextLine}</p>
                  )}
                </div>
              </Panel>
            )
          ...
          <Panel title="Cycle Timeline" icon={History}
            headerExtra={<span className="text-[11px] border border-slate-700 px-1.5 py-0.5 text-slate-300 tracking-[0.02em] font-mono" style={{ fontFamily: DATA_FONT }}>5Y</span>}>
            <div className="space-y-0.5 pt-0.5">
              {CYCLE_TIMELINE.map((item, i) => {
                ...
                return (
                  <div key={i} className={`flex items-start gap-[10px] p-2 rounded-sm transition-colors ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-[6px]">
                        <span className={`text-[11px] font-mono truncate ${item.now ? 'text-slate-200' : 'text-slate-300 tracking-[0.02em]'}`}>{item.range}</span>
                        {item.now && <span className="text-[11px] font-bold text-blue-400 shrink-0">Now</span>}
                      </div>
                      <div className="flex items-center justify-between mt-0.5">
                        <span className={`text-[11px] font-bold ${item.now ? '' : 'text-slate-300 tracking-[0.02em]'}`}
                          style={{ color: item.now ? phaseColor : undefined }}>{item.phase}</span>
                        <span className={`text-[11px] font-mono ${item.now ? 'text-slate-300 tracking-[0.02em]' : 'text-slate-300 tracking-[0.02em]'}`}>{item.pct}%</span>
                      </div>
                    </div>
                  </div>
                )
        ...
        <section className="w-1/2 flex flex-col gap-[10px]">
          ...
          <Panel title="Analysis Engine Core" className="flex-none">
            ...
            {centerTab === 'CYCLE VIEW' && (
              <div className="flex items-center justify-between shrink-0 mb-1">
                <div className="flex items-center gap-0.5">
                  <span className="text-[11px] text-slate-400 tracking-[0.02em] mr-1">Zoom</span>
                  {['1M','3M','6M','YTD','1Y','3Y','5Y','MAX'].map(z => (
                    <button key={z} onClick={() => setZoom(z)}
                      className={`px-1.5 py-0.5 text-[11px] transition-colors ${
                        zoom === z ? 'text-blue-400 border border-blue-500/40 bg-blue-500/10' : 'text-slate-400 tracking-[0.02em] hover:text-slate-300 tracking-[0.02em]'
                      }`}>{z}</button>
                  ))}
                </div>
                <div className="flex items-center gap-[6px].5 text-[11px] text-slate-400 tracking-[0.02em]">
                  <span>Scale</span>
                  <span className="px-1.5 py-0.5 border border-slate-700 cursor-pointer">Linear</span>
                  <RotateCcw size={11} className="cursor-pointer hover:text-slate-300 tracking-[0.02em]" />
                </div>
              </div>
            )}
            ...
            {/* [1] Relative Spread vs SOXX: selected buckets stronger or weaker than SOXX */}
            <div className="mb-2">
              <div className="flex items-center gap-[14px] mb-1 shrink-0 flex-wrap">
                <span className="text-[11px] font-bold text-slate-300 tracking-[0.02em] uppercase tracking-widest" style={{ fontFamily: UI_FONT }}>Relative Spread vs SOXX</span>
                {[
                  ...
                  <span key={l.label} className="flex items-center gap-[6px] text-[11px] text-slate-400 tracking-[0.02em]">
                    <span className="inline-block w-3 h-0.5 rounded" style={{ backgroundColor: l.color }} />
                    {l.label}
                  </span>
                ))}
              </div>
              <p className="text-[11px] text-slate-400 tracking-[0.02em] mb-1">{CHART_HELPER_COPY.relativeSpread}</p>
              <div className="h-[220px] w-full">
                {spreadData.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-[14px] font-medium leading-[1.6] text-slate-500">Data pending</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={spreadData} margin={{ top: 2, right: 12, bottom: 0, left: -18 }}>
                      <CartesianGrid strokeDasharray="2 4" stroke="#1e293b" vertical={false} />
                      <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#c9cdd4', fontSize: 11 }} interval="preserveStartEnd" />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: '#c9cdd4', fontSize: 11 }}
                        tickFormatter={v => `${(+v) >= 0 ? '+' : ''}${(+v).toFixed(0)}`} />
                      <Tooltip contentStyle={{ backgroundColor: '#0c1220', border: '1px solid #1e293b', fontSize: 11 }}
                        formatter={(v: unknown, name: unknown) => [
                          `Relative spread: ${(+String(v)) >= 0 ? '+' : ''}${(+String(v)).toFixed(1)}% vs SOXX`,
                          String(name),
                        ]} />
                      <ReferenceLine y={0} stroke="#3b82f6" strokeDasharray="3 2" strokeOpacity={0.6} label={{ value: 'SOXX', position: 'insideTopRight', fontSize: 11, fill: '#3b82f6' }} />
            ...
            {/* [2] Rebased Bucket Flow: same-start comparison of selected buckets */}
            <div className="mb-2 border-t border-slate-800/60 pt-2">
              <div className="flex items-center gap-[14px] mb-1 shrink-0 flex-wrap">
                <span className="text-[11px] font-bold text-slate-300 tracking-[0.02em] uppercase tracking-widest" style={{ fontFamily: UI_FONT }}>Rebased Bucket Flow</span>
                {[
              ...
              <p className="text-[11px] text-slate-400 tracking-[0.02em] mb-1">{CHART_HELPER_COPY.rebasedFlow}</p>
              <div className="h-[220px] w-full">
                {rebasedData.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-[14px] font-medium leading-[1.6] text-slate-500">Loading…</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={rebasedData} margin={{ top: 2, right: 12, bottom: 0, left: -18 }}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: '#0c1220', border: '1px solid #1e293b', fontSize: 11 }}
                        formatter={(v: unknown, name: unknown) => [
                          `Same-start move: ${(+String(v)) >= 0 ? '+' : ''}${(+String(v)).toFixed(1)}%`,
                          String(name),
                        ]} />
            ...
            {(() => {
              ...
              return (
                <div className="border-t border-slate-800/60 pt-2">
                  <div className="text-[11px] font-bold text-slate-300 tracking-[0.02em] uppercase tracking-widest mb-0.5" style={{ fontFamily: UI_FONT }}>Capital Flow Stage</div>
                  <p className="text-[11px] text-slate-400 tracking-[0.02em] mb-2">Shows how far AI-related capital has spread across the semiconductor value chain.</p>
                  <div className="flex items-start gap-[6px]">
                    {FLOW.map((f, i) => (
                      <div key={f.label} className="flex items-center gap-[6px] min-w-0">
                        <div className="flex flex-col items-center gap-0.5 min-w-0">
                          <div className="text-[11px] text-slate-400 tracking-[0.02em] text-center leading-[1.6] truncate w-full text-center">{f.label}</div>
                          <div className={`text-[11px] font-bold uppercase px-1.5 py-0.5 border rounded-sm whitespace-nowrap ${STAGE_CLS[f.stage]}`}>
                            {f.stage}
                          </div>
                          {f.spread !== null && (
                            <div className="text-[11px] font-mono text-slate-400 tracking-[0.02em]" style={{ fontFamily: DATA_FONT }}>
                              {f.spread >= 0 ? '+' : ''}{f.spread.toFixed(1)}pp
                            </div>
                          )}
                        </div>
                        {i < FLOW.length - 1 && (
                          <span className="text-[11px] text-slate-500 shrink-0 mt-4">→</span>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-800">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>CYCLE INDICATOR (Composite)</span>
                <div className="flex items-center gap-[14px] text-[11px] text-slate-400 tracking-[0.02em]">
                  <span className="flex items-center gap-[6px]"><span className="inline-block w-3 h-0.5 bg-red-500" /> High Zone</span>
                  <span className="flex items-center gap-[6px]"><span className="inline-block w-3 h-0.5 bg-emerald-500" /> Low Zone</span>
                  <span className={`text-[14px] font-medium leading-[1.6] font-bold px-1.5 py-0 border ${
                    currentComp === null ? 'text-slate-400 tracking-[0.02em] border-slate-700' :
              ...
              <div className="h-[110px]">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={visibleData} margin={{ top: 4, right: 12, bottom: 0, left: -18 }}>
                    ...
                    <YAxis domain={([min, max]: [number, number]) => {
            ...
            <div className="mt-2 pt-2 border-t border-slate-800">
              <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>CYCLE PHASE PROBABILITY</span>
              {phaseProb === null ? (
                <div className="mt-1.5 h-6 flex items-center text-[14px] font-medium leading-[1.6] text-slate-500">Loading…</div>
              ) : (
            ...
            {centerTab === 'BREADTH' && (() => {
              ...
              const confImpactCls =
                confImpact === 'Confirms' ? 'text-emerald-400' :
                ...
                <div className="space-y-3 pt-1">
                  ...
                  <div className="grid grid-cols-3 gap-[10px]">
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>Breadth Score</div>
                      <div className={`text-[20px] font-black font-mono leading-none ${bScore !== null ? bStateCls : 'text-slate-500'}`}>
                        {bScore !== null ? bScore : '?'}
                      </div>
                    </div>
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>Breadth State</div>
                      <div className={`text-[14px] font-black leading-none mt-1 ${bStateCls}`}>
                        {bState}
                      </div>
                    </div>
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>Confidence Impact</div>
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em] mb-2" style={{ fontFamily: UI_FONT }}>Advancing / Declining</div>
                    {!live ? (
                      <div className="text-[14px] font-medium leading-[1.6] text-slate-500 py-3 text-center">Loading…</div>
                    ) : !advDecValid ? (
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="grid grid-cols-3 text-[11px] font-mono" style={{ fontFamily: DATA_FONT }}>
                          <div>
                          ...
                          <div className="text-center">
                            ...
                            <span className={`font-bold ${netBreadth !== null && netBreadth > 0 ? 'text-emerald-400' : netBreadth !== null && netBreadth < 0 ? 'text-red-400' : 'text-slate-400 tracking-[0.02em]'}`}>
                        ...
                        <div className="text-[11px] text-slate-400 tracking-[0.02em] mt-0.5">
                          % above 20MA: <span className="text-slate-300 tracking-[0.02em] font-bold">{breadthPct}%</span>
                          <span className="ml-2 text-slate-400 tracking-[0.02em]">夷?{breadthLabel}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em] mb-2" style={{ fontFamily: UI_FONT }}>Participation Health</div>
                    <div className="grid grid-cols-3 gap-[6px].5 text-[11px] font-mono" style={{ fontFamily: DATA_FONT }}>
                      {([
                        ...
                        return (
                          <div key={label} className="bg-slate-900/40 border border-slate-800 p-2 text-center">
                            <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>{label}</div>
                    ...
                    return (
                      <div className="border-t border-slate-800 pt-2">
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>AI Concentration Trend</span>
                          <span className={`text-[11px] font-black ${trendCls}`}>{trendIcon} {trend}</span>
                        </div>
                        <div className="flex items-center gap-[14px] mt-1 text-[11px] font-mono" style={{ fontFamily: DATA_FONT }}>
                          <span className="text-slate-500">{first.toFixed(1)}% <span className="text-slate-400 tracking-[0.02em]">→</span> <span className={trendCls}>{last.toFixed(1)}%</span></span>
                          <span className="text-slate-400 tracking-[0.02em]">|</span>
                  ...
                  <div className={`border-t border-slate-800 pt-2`}>
                    <div className="text-[20px] font-semibold font-sans text-slate-400 tracking-[0.02em] mb-1.5">Breadth Interpretation</div>
                    <div className={`p-3 rounded-sm border text-[11px] ${
                      ...
                      <div className="flex items-center gap-[6px].5 mb-0.5">
                        <span className={`font-bold text-[11px] uppercase px-1 rounded-sm ${
                          isBreadthConflict ? 'bg-orange-500/20 text-orange-400' : 'bg-slate-800 text-slate-400 tracking-[0.02em]'
                        }`}>{displayConflict(conflictType)}</span>
                      </div>
                      <span className="text-slate-300 tracking-[0.02em] leading-[1.8]">{breadthInterpretation}</span>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="border-t border-slate-800 pt-2">
                        <div className="flex items-center justify-between mb-1">
                          <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>Breadth History</div>
                          {bhData.length > 0 && (
            ...
            {centerTab === 'MOMENTUM' && (() => {
              ...
              const momRegime =
                ...
                marketRegime === 'NEUTRAL'  && momDir === 'Fading'        ? 'Fading'        :
              ...
              const momInterpretation =
                conflictType === 'MOMENTUM_DIVERGENCE'
                  ? 'Price trend remains positive, but momentum is softening.'
                : conflictType === 'AI_DISTORTION'
                  ? 'Momentum is concentrated in AI infrastructure leaders.'
                : conflictType === 'AI_INFRA_SUSTAINABILITY_RISK'
                  ? 'Momentum strength is concentrated in AI infrastructure leadership.'
                : (conflictType === 'NO_CONFLICT' || conflictType === '?')
              ...
              return (
                <div className="space-y-3 pt-1">
                  ...
                  <div className="grid grid-cols-2 gap-[10px]">
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>Momentum Signal</div>
                      ...
                      <div className={`text-[11px] mt-0.5 font-bold ${momDirCls}`}>{momDir}</div>
                    </div>
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>Momentum Regime</div>
                      <div className={`text-[14px] font-black leading-none mt-1 ${momDirCls}`}>
                        {live ? momRegime : '?'}
                      </div>
                    </div>
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>RSI 14</div>
                      {(() => {
                    ...
                    <div className="bg-slate-900/40 border border-slate-800 p-3">
                      <div className="text-[11px] text-slate-500 uppercase tracking-[0.12em] font-semibold text-[11px] mb-1" style={{ fontFamily: UI_FONT }}>MACD</div>
                      {(() => {
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-3 gap-[6px].5 -mt-1">
                        {rocs.map(({ label, val }) => {
                          ...
                          return (
                            <div key={label} className="bg-slate-900/40 border border-slate-800 p-2 text-center">
                              ...
                              <div className={`text-[13px] font-bold font-mono ${cls}`}>
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="flex items-center gap-[10px] mb-1.5">
                      <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>Bucket Momentum Ranking</span>
                      <span className="text-[11px] text-slate-400 tracking-[0.02em] bg-slate-800/60 px-1 rounded-sm">1M return proxy</span>
                    </div>
                    {!periodReturns ? (
                      <div className="text-[14px] font-medium leading-[1.6] text-slate-500 py-3 text-center">Data pending</div>
                    ) : (
                      <div>
                        ...
                        {/* SOXX baseline row */}
                        <div className="grid items-center py-1 border-b border-slate-800/30 text-[14px] font-medium leading-[1.6] font-mono"
                          style={{ fontFamily: DATA_FONT,  gridTemplateColumns: '1fr 52px 56px 80px' }}>
                          <span className="flex items-center gap-[6px].5">
                            <span className="w-2 h-2 rounded-sm shrink-0 bg-blue-500" />
                            <span className="text-slate-400 tracking-[0.02em]">SOXX Index</span>
                          </span>
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em] mb-1.5" style={{ fontFamily: UI_FONT }}>Momentum Conflict Detector</div>
                    <div className={`p-3 rounded-sm border text-[11px] ${
                      isMomConflict ? 'border-orange-500/20 bg-orange-500/5' : 'border-slate-800'
                    }`}>
                      <div className="flex items-center gap-[6px].5 mb-0.5">
                        <span className={`font-bold text-[11px] uppercase px-1 rounded-sm ${
                          isMomConflict ? 'bg-orange-500/20 text-orange-400' : 'bg-slate-800 text-slate-400 tracking-[0.02em]'
                        }`}>{displayConflict(conflictType)}</span>
                      </div>
                      <span className="text-slate-300 tracking-[0.02em] leading-[1.8]">{momInterpretation}</span>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="border-t border-slate-800 pt-2">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>Momentum History</span>
                          {mhData.length > 0 && (
            ...
            {centerTab === 'MAP' && (() => {
              ...
              const tiles = BUCKET_DEFS.map(b => {
                const mapping = getSemiconductorBucketMapping(b.key)
                const ret1m = periodReturns?.[b.key]?.['1M'] ?? null
                ...
                // Signal/risk tag: check if this bucket matches primary signal/risk
                ...
                const tag = isDriver ? 'LEADER' : isRisk ? 'RISK' : null
                const tagCls = isDriver ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                return { ...b, mapping, ret1m, ret5d, rel, dir, dirCls, tag, tagCls }
              })
              ...
              const mapInterpretation = (() => {
                const driver = primaryDriver !== '?' ? `${primaryDriver} is the primary relative signal.` : ''
              ...
              return (
                <div className="space-y-3 pt-1">
                  ...
                  <div>
                    <div className="flex items-center gap-[10px] mb-1.5">
                      <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>Bucket Map</span>
                      {live?.bucket_weights
                        ? <span className="text-[11px] text-emerald-600 bg-emerald-900/20 px-1 rounded-sm">market-cap weighted</span>
                        : <span className="text-[11px] text-slate-400 tracking-[0.02em] bg-slate-800/60 px-1 rounded-sm">equal-size</span>
                      }
                    </div>
                    {/* Weight bar */}
                    {live?.bucket_weights && (() => {
                    ...
                    {!periodReturns ? (
                      ...
                      <div className="grid grid-cols-2 gap-[6px].5">
                        {/* SOXX reference tile */}
                        <div className="col-span-2 border border-blue-500/30 bg-slate-900/50 rounded-sm p-3 flex items-center justify-between">
                          <div className="flex items-center gap-[10px]">
                            <span className="w-2 h-2 rounded-sm shrink-0 bg-blue-500" />
                            <span className="text-[13px] font-bold text-slate-200">SOXX Index</span>
                            <span className="text-[11px] text-slate-400 tracking-[0.02em] bg-slate-800/60 px-1 rounded-sm">benchmark</span>
                          </div>
                          <div className="flex items-center gap-[14px]">
                            <div className="text-right">
                              <div className="text-[11px] text-slate-500">1M</div>
                              <div className={`text-[14px] font-black font-mono tabular-nums ${retColor(soxx1M)}`}>{fmtRet(soxx1M)}</div>
                            </div>
                            <div className="text-right">
                              <div className="text-[11px] text-slate-400 tracking-[0.02em]">Score</div>
                              <div className="text-[14px] font-black font-mono text-blue-400" style={{ fontFamily: DATA_FONT }}>{engineScore}</div>
                            </div>
                          </div>
                        </div>
                        {/* Bucket tiles */}
                        {tiles.map(b => {
                          ...
                          const displayName = b.mapping?.label ?? b.name
                          return (
                          <div key={b.name} className={`border ${b.accentCls} bg-slate-900/50 rounded-sm p-3`}>
                            <div className="flex items-center justify-between mb-1.5">
                              <div className="flex items-center gap-[6px].5">
                                <span className="w-2 h-2 rounded-sm shrink-0" style={{ backgroundColor: b.color }} />
                                <span className="text-[13px] font-bold text-slate-200 truncate">{displayName}</span>
                                {bwPct && <span className="text-[11px] text-slate-400 tracking-[0.02em] font-mono" style={{ fontFamily: DATA_FONT }}>{bwPct}</span>}
                              </div>
                              {b.tag && (
                                <span className={`text-[11px] font-black px-1 rounded-sm ${b.tagCls}`}>{b.tag}</span>
                              )}
                            </div>
                            {b.mapping && (
                              <div
                                className="text-[11px] text-slate-400 tracking-[0.02em] truncate mb-1"
                                title={`${formatBucketClassificationHint(b.mapping)} | ${b.mapping.note}`}
                              >
                                {formatBucketTickerHint(b.mapping)} | {b.mapping.driverClass === 'internal_driver' ? 'Internal Driver' : 'Context Only'}
                              </div>
                            )}
                            <div className="grid grid-cols-2 gap-[6px] font-mono" style={{ fontFamily: DATA_FONT }}>
                              <div>
                                <div className="text-[11px] text-slate-400 tracking-[0.02em]">1M</div>
                                <div className={`text-[13px] font-black tabular-nums ${retColor(b.ret1m)}`}>{fmtRet(b.ret1m)}</div>
                              </div>
                              <div>
                                <div className="text-[11px] text-slate-400 tracking-[0.02em]">vs SOXX</div>
                                <div className={`text-[13px] font-black tabular-nums ${retColor(b.rel)}`}>{b.rel !== null ? fmtRet(b.rel) : '?'}</div>
                              </div>
                            </div>
                            <div className={`text-[13px] font-bold mt-1 ${b.dirCls}`}>{b.dir}</div>
                          </div>
                        )})}
                      </div>
                    )}
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em] mb-2" style={{ fontFamily: UI_FONT }}>Cycle Position</div>
                    <div className="flex gap-[6px]">
                      {CYCLE_STAGES.map((s, i) => {
                        ...
                        return (
                          <div key={s} className={`flex-1 text-center py-1 rounded-sm text-[11px] font-bold border transition-colors ${
                            isCurrent
                              ? 'border-blue-500 bg-blue-500/20 text-blue-300'
                              : 'border-slate-800 text-slate-400 tracking-[0.02em]'
                          }`}>
                            {s.replace(' ', '?')}
                          </div>
                        )
                    ...
                    <div className="flex justify-between text-[11px] text-slate-400 tracking-[0.02em] mt-1">
                      <span>Engine Score: <span className="text-slate-300 tracking-[0.02em] font-bold">{live ? engineScore : '?'}</span></span>
                  ...
                  <div className="border-t border-slate-800 pt-2">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[11px] font-semibold font-sans text-slate-400 tracking-[0.02em] uppercase tracking-[0.12em]" style={{ fontFamily: UI_FONT }}>AI Concentration Overlay</span>
                      {(() => {
                        const hist  = live?.ai_infra_concentration_history ?? []
                        const valid = hist.filter(r => r.top5_weight !== null)
                        if (valid.length < 2) return null
                        ...
                        return <span className={`text-[11px] font-black px-1 rounded-sm ${cls}`}>Dominance {trend}</span>
                      })()}
                    </div>
                    <div className="grid grid-cols-2 gap-[10px] mb-2">
                      <div className="bg-slate-900/40 border border-slate-800 p-2 text-center">
                        <div className="text-[11px] text-slate-400 tracking-[0.02em] uppercase">Top 5 Concentration</div>
                      ...
                      <div className="bg-slate-900/40 border border-slate-800 p-2 text-center">
                        <div className="text-[11px] text-slate-400 tracking-[0.02em] uppercase">EW vs CW Spread</div>
                        <div className={`text-[16px] font-black font-mono ${
                          ewVsCw === null ? 'text-slate-500' :
                          ewVsCw < -20   ? 'text-red-400'    :
                          ewVsCw < -10   ? 'text-orange-400' : 'text-slate-300 tracking-[0.02em]'
                    ...
                    <div className={`p-3 rounded-sm border text-[14px] font-medium leading-[1.6] ${
                      conflictType === 'AI_DISTORTION' || conflictType === 'AI_INFRA_SUSTAINABILITY_RISK'
                        ? 'border-orange-500/20 bg-orange-500/5' : 'border-slate-800'
                    }`}>
                      <span className="text-slate-300 tracking-[0.02em] leading-[1.8]">{aiOverlay}</span>
                    </div>
                  </div>

                  {/* 4. Map Interpretation */}
                  <div className="border-t border-slate-800 pt-2">
                    <div className="text-[20px] font-semibold font-sans text-slate-400 tracking-[0.02em] mb-1.5">Map Interpretation</div>
                    <div className={`p-3 rounded-sm border text-[14px] font-medium leading-[1.6] ${
                      hasConflict ? 'border-orange-500/20 bg-orange-500/5' : 'border-slate-800'
                    }`}>
                      <div className="flex items-center gap-[6px].5 mb-0.5">
                        <span className={`font-bold text-[11px] uppercase px-1 rounded-sm ${
                          hasConflict ? 'bg-orange-500/20 text-orange-400' : 'bg-slate-800 text-slate-400 tracking-[0.02em]'
                        }`}>{displayConflict(conflictType)}</span>
                      </div>
                      <span className="text-slate-300 tracking-[0.02em] leading-[1.8]">{mapInterpretation}</span>
                    </div>
                  </div>
