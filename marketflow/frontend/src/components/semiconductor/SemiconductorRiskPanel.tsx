...
const KPIBox = ({ label, value, subValue, state }: {
  ...
  return (
    <div className="flex flex-col border-r border-slate-800 px-6 first:pl-0 last:border-0 min-w-[140px]">
      <span className="text-[11px] font-bold text-slate-500 uppercase tracking-tighter mb-1">{label}</span>
...
const SectionHeader = ({ title, icon: Icon }: { title: string; icon?: any }) => (
  <div className="flex items-center gap-[10px] mb-4 border-b border-slate-800 pb-2">
    {Icon && <Icon size={14} className="text-blue-500" />}
    <h3 className="text-[11px] font-bold text-slate-300 uppercase tracking-widest" style={{ fontFamily: UI_FONT }}>{title}</h3>
  </div>
);
    ...
    <div className="flex h-screen flex-col bg-[#050810] font-sans text-slate-300 select-none">
      ...
      <header className="flex items-center justify-between border-b border-slate-800 bg-[#0a0f1e] px-6 py-3 shrink-0">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-[10px] mr-4">
        ...
        <div className="flex items-center gap-[18px] text-slate-500 font-mono text-[11px]" style={{ fontFamily: DATA_FONT }}>
      ...
      <nav className="flex bg-[#0a0f1e] border-b border-slate-800 px-6 shrink-0">
        {['ENGINE', 'DATA LAB', 'STRATEGY'].map(tab => (
          <button
            ...
            className={`px-8 py-3 text-[11px] font-bold tracking-[0.2em] transition-all border-b-2 ${
              activeTab === tab
        ...
        <aside className="w-1/4 border-r border-slate-800 overflow-y-auto p-5 space-y-6">
            ...
            <Card className="bg-blue-500/5 border-blue-500/20">
              <div className="flex items-center gap-[14px] mb-4">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-slate-500">Regime stability: HIGH</div>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between py-2 border-b border-slate-800/50">
                <span className="text-[11px] text-slate-500">Fed Funds Rate</span>
                <span className="text-[11px] font-mono font-bold text-white" style={{ fontFamily: DATA_FONT }}>{macro.rates}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-800/50">
                <span className="text-[11px] text-slate-500">Global Liquidity</span>
                <span className="text-[11px] font-mono font-bold text-red-400" style={{ fontFamily: DATA_FONT }}>{macro.liquidity}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-800/50">
                <span className="text-[11px] text-slate-500">Inventory Status</span>
                <span className="text-[11px] font-mono font-bold text-blue-400" style={{ fontFamily: DATA_FONT }}>{macro.inventory_state}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className={`p-2 border-l-2 text-[11px] ${kpis.risk === 'ENTER' ? 'border-emerald-500 bg-emerald-500/5' : 'border-slate-700'}`}>
                <span className="font-black block mb-1 uppercase">ENTER (Bullish)</span>
                Structural expansion + High Demand
              </div>
              <div className={`p-2 border-l-2 text-[11px] ${kpis.risk === 'CAUTION' ? 'border-orange-500 bg-orange-500/5' : 'border-slate-700'}`}>
                <span className="font-black block mb-1 uppercase">CAUTION (Watch)</span>
                Valuation heat + Momentum divergence
              </div>
              <div className={`p-2 border-l-2 text-[11px] ${kpis.risk === 'AVOID' ? 'border-red-500 bg-red-500/5' : 'border-slate-700'}`}>
            ...
            <>
              <Card>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex items-center gap-[18px] text-[11px] font-mono" style={{ fontFamily: DATA_FONT }}>
                    <span className="flex items-center gap-[6px] text-blue-400"><span className="h-1.5 w-1.5 bg-blue-400" /> SOXX</span>
                    <span className="flex items-center gap-[6px] text-slate-500"><span className="h-1.5 w-1.5 bg-slate-500" /> BUCKET</span>
                  </div>
                </div>
                <div className="h-[320px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={history.slice(Math.max(0, inspectionIndex - 30), inspectionIndex)}>
                      ...
                      <YAxis domain={['auto', 'auto']} axisLine={false} tickLine={false} tick={{fill: '#475569', fontSize: 11}} />
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-800 pt-4">
                   <div className="text-[11px]">
                      ...
                      <span className="font-mono font-bold text-white" style={{ fontFamily: DATA_FONT }}>{signals.spread} pts</span>
                   </div>
                   <div className="text-[11px]">
                      <span className="text-slate-500 uppercase">Correlation: </span>
                      <span className="font-mono font-bold text-emerald-500" style={{ fontFamily: DATA_FONT }}>{signals.correlation}</span>
                   </div>
                </div>
              </Card>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <p className="text-[11px] text-slate-600 mt-4 uppercase tracking-tighter">Cluster analysis: Positive drift detected in top-tier logic bucket</p>
                </Card>
              </div>
            </>
          )}
              ...
              <Card>
                ...
                <div className="grid grid-cols-3 gap-6 pt-4">
                  <div className="text-center p-4 border border-slate-800 bg-slate-900/20">
                    <div className="text-[11px] text-slate-500 uppercase font-black">Bear</div>
                    <div className="text-2xl font-mono font-black text-red-500" style={{ fontFamily: DATA_FONT }}>12%</div>
                  </div>
                  <div className="text-center p-4 border border-blue-500/30 bg-blue-500/10 scale-105">
                    <div className="text-[11px] text-blue-400 uppercase font-black">Base</div>
                    <div className="text-2xl font-mono font-black text-white" style={{ fontFamily: DATA_FONT }}>65%</div>
                  </div>
                  <div className="text-center p-4 border border-slate-800 bg-slate-900/20">
                    <div className="text-[11px] text-slate-500 uppercase font-black">Bull</div>
                    <div className="text-2xl font-mono font-black text-emerald-500" style={{ fontFamily: DATA_FONT }}>23%</div>
                  </div>
                </div>
              </Card>
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-3 border border-slate-800/50 hover:bg-white/5 transition-colors cursor-pointer">
                      <div className="flex items-center gap-[14px]">
                        ...
                        <span className="text-[11px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-400">Match: {analog.match}</span>
                      </div>
                      <span className="text-[11px] font-bold text-slate-500">{analog.result}</span>
                    </div>
                  ))}
                </div>
              </Card>
        ...
        <aside className="w-1/4 border-l border-slate-800 overflow-y-auto p-5 space-y-6">
          <div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center p-2 bg-slate-900/40 border border-slate-800">
                <span className="text-[11px] text-slate-500 uppercase">Breadth (Adv/Dec)</span>
                <span className="text-[11px] font-bold text-emerald-500">{signals.breadth.ratio}</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-slate-900/40 border border-slate-800">
                <span className="text-[11px] text-slate-500 uppercase">Liquidity Signal</span>
                <span className="text-[11px] font-bold text-red-400">TIGHTENING</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-slate-900/40 border border-slate-800">
                <span className="text-[11px] text-slate-500 uppercase">Volatility (Implied)</span>
          ...
          <div>
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody className="divide-y divide-slate-800/30 font-mono" style={{ fontFamily: DATA_FONT }}>
          ...
          <Card className="border-orange-500/20 bg-orange-500/5">
            <h4 className="text-[11px] font-black text-orange-500 uppercase mb-2 flex items-center gap-[10px]">
      ...
      <footer className="border-t border-slate-800 bg-[#0a0f1e] px-6 py-4 shrink-0">
        <div className="flex flex-col gap-[14px]">
          <div className="flex justify-between items-center text-[11px] font-mono font-bold text-slate-500" style={{ fontFamily: DATA_FONT }}>
          ...
          <div className="flex justify-between items-center text-[11px] text-slate-600 uppercase tracking-widest font-mono" style={{ fontFamily: DATA_FONT}}>
            <span>2024-01-01</span>
            <div className="flex gap-[18px]">
              <span className="flex items-center gap-[6px]"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> SYNCED</span>
