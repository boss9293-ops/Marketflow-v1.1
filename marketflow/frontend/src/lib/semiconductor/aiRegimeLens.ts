...
export function computeAIRegimeLens(raw: MarketDataInput): AIRegimeLens {
  ...
  // tier2 bonus for memory (짹1pp per signal)
  const tier2Bonus = (() => {
  ...
  const memory: AIRegimeComponentState = (() => {
    ...
    if (ma > 0.03)  return { state: 'CONFIRMED',     signal: sig, spread: pp, sources: ['MU'],
      note: 'Memory segment participation is aligned with AI-linked demand context through price structure.' }
  ...
  // ?? Component 3: Foundry / Packaging Confirmation ??????????????????????????????
  const foundry: AIRegimeComponentState = (() => {
    ...
    if (fs > 0.03)  return { state: 'CONFIRMING', signal: sig, spread: pp, sources: ['TSM'],
      note: 'Foundry segment is outperforming the benchmark, consistent with sustained AI-linked production context.' }
    ...
    return              { state: 'LAGGING',   signal: sig, spread: pp, sources: ['TSM'],
      note: 'Foundry segment participation is lagging the benchmark, indicating weaker selected-bucket confirmation.' }
  })()
  ...
  const regime_label: AIRegimeLabel = (() => {
    ...
    if (ai_infra.state === 'LEADING' && memory.state === 'CONFIRMED' && foundry.state === 'CONFIRMING' && rotation_risk.state === 'BROAD') return 'AI_LED_BROAD'
