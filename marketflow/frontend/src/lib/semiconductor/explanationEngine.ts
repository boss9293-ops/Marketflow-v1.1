 ...
 *
 * DESIGN RULES:
 *   - No trade-entry language: use risk and interpretation context, not direct action calls
...
const HEADLINE: Record<CycleState, string> = {
  ...
  'Peak Risk':      'Semiconductor sector shows peak-risk conditions ??signal breadth fading.',
...
const HEADLINE_WITH_CONFLICT: Partial<Record<ConflictTypeV2, string>> = {
  AI_DISTORTION:               'Semiconductor sector is rising, but the advance is narrow ??AI-linked leadership is concentrated.',
  ...
  AI_INFRA_SUSTAINABILITY_RISK:'AI Infrastructure is showing extreme outperformance, but leadership concentration is at structural risk levels.',
...
function buildSubscriberExplanation(
  ...
  const conflictNote: Partial<Record<ConflictTypeV2, string>> = {
    AI_DISTORTION:
      'Semiconductor index strength is concentrated in a small group of AI-linked names like NVDA. ' +
    ...
    MOMENTUM_DIVERGENCE:
      'Price is rising but the underlying momentum (RSI, MACD) is fading. ' +
