import type { AIRegimeLens, AIRegimeLabel } from './aiRegimeLens'
...
export function detectAlignment(input: EngineOutput): AlignmentResult {
  ...
  const mixedPairs: Array<[boolean, string]> = [
    ...
    [map === 'neutral' && momentum === 'strong',
     'Momentum is strong but market structure remains transitional rather than fully stable.'],
    [ai_concentration === 'high' && breadth !== 'weak',
...
export function buildStructuralInterpretation(
  ...
  // Aligned + empty support means signals are consistently weak/neutral — not "broadly supported"
  const baseStatement =
    alignmentResult.alignment === 'Aligned' && support.length === 0
      ? 'The structure shows consistent but limited directional signal, with constraints present'  :
    alignmentResult.alignment === 'Aligned'
      ? 'The structure is broadly aligned, while constraints remain limited'                        :
    alignmentResult.alignment === 'Mixed'
      ? 'The structure is partially aligned, while key constraints limit full participation'        :
  ...
  const relationship =
    support.length > 0 && weakness.length > 0 ? `Observed relative strength reflects ${join(support)}, while constraints include ${join(weakness)}.` :
    support.length > 0                         ? `Observed relative strength reflects ${join(support)}.`                                           :
    weakness.length > 0                        ? `Structural constraints include ${join(weakness)}.`                                             :
                                                 'Signal direction is consistent but lacks strong directional confirmation.'

  return { structure, relationship, support, weakness }
}

// ── Main function ─────────────────────────────────────────────────────────────

const STRUCTURAL_BEHAVIOR: Record<AlignmentResult['alignment'], string> = {
  Aligned:   'broad structural alignment and consistent participation',
  Mixed:     'partial alignment with uneven participation across segments',
  Divergent: 'narrow leadership and conflicting internal signals',
}

const REGIME_CONTEXT_MAP: Record<AIRegimeLabel, string> = {
  AI_LED_BROAD:   'AI infrastructure leadership is confirmed across multiple semiconductor segments, with broad participation aligned with the structural advance.',
  ...
  CONTRACTION:    'Structural weakness is confirmed across all semiconductor segments; no selected bucket is offsetting the relative weakness.',
}
...
export function translateEngineOutput(input: EngineOutput): InterpretationOutput {
  ...
  const regime_context = input.ai_regime
    ? REGIME_CONTEXT_MAP[input.ai_regime.regime_label]
    : undefined
  ...
  return {
    ...
    interpretation: `${structural.structure} ${structural.relationship}`,
    context,
    confidence,
    regime_context,
  }
