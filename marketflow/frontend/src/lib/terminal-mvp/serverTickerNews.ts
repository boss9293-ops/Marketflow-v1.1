...
// ?? Build payload from Yahoo Finance RSS items (AM/PM bucket) ?????????????????????????
...
// ?? Main export: Alpaca first, Yahoo Finance fallback ?????????????????????????????????
export async function fetchTickerNewsFromYahoo(symbol: string, dateET: ETDateString): Promise<BuiltTickerNewsPayload> {
  ...
  if (freshTimeline.length === 0) {
    try {
      ...
      if (!res) throw new Error('Yahoo Finance RSS request failed after retries')
